...
function add_user_role() {
  ...
  curl_result=$(
    curl -w $"\n%{http_code}" --silent -X PATCH "${IDAM_URL}/users/${USER}/roles/${ROLE}" -H "accept: */*" -H "authorization:Bearer ${IDAM_ACCESS_TOKEN}"
  )
...
${response_body}"
    ...
    # format a response for low level curl error (e.g. exit code 7 = 'Failed to connect() to host or proxy.')
    response="CURL-${exit_code}
ERROR: User ${USER} role ${ROLE} update request has failed with curl exit code: ${exit_code}"
  fi
  echo "$response"
}
...
function get_role_id() {
  ...
  curl_result=$(
    curl -w $"\n%{http_code}" --silent -X GET "${IDAM_URL}/roles/name/${ROLENAME}" -H "accept: */*" -H "authorization:Bearer ${IDAM_ACCESS_TOKEN}"
  )
...
${response_body}"
      echo "HTTP-${response_status}
ERROR: Unable to find role with the name ${ROLENAME} failed with http response: HTTP-${response_status}"
    fi
  else
    # format a response for low level curl error (e.g. exit code 7 = 'Failed to connect() to host or proxy.')
    response="CURL-${exit_code}
ERROR: Unable to find role with the name ${ROLENAME} failed with curl exit code: ${exit_code}"
  fi
  echo "$response"
}
...
function process_input_file() {
  ...
  local caseworker=$(get_role_id "caseworker")
  local caseworker_id=$(echo $caseworker | jq --raw-output '.id')
  local caseworker_cmc=$(get_role_id "caseworker-cmc")
  local caseworker_cmc_id=$(echo $caseworker_cmc | jq --raw-output '.id')
  local caseworker_cmc_judge=$(get_role_id "caseworker-cmc-judge")
  local caseworker_cmc_judge_id=$(echo $caseworker_cmc_judge | jq --raw-output '.id')
  local idam_mfa_disabled=$(get_role_id "idam-mfa-disabled")
  local idam_mfa_disabled_id=$(echo $idam_mfa_disabled | jq --raw-output '.id')
    ...
    while IFS= read -r user; do
      ...
      if [ "$inviteStatus" != "SUCCESS" ]; then
        ...
        if [ "$operation" == "add" ]; then
          ...
          if [[ $rawReturnedValue != *"HTTP-"* ]]; then 
            ...
            if [[ $submit_response != *"HTTP-"* ]]; then 
              submit_response=$(add_user_role "$userId" "$caseworker_id")
              if [[ $submit_response != *"HTTP-"* ]]; then 
                submit_response=$(add_user_role "$userId" "$caseworker_cmc_id")
                if [[ $submit_response != *"HTTP-"* ]]; then 
                  submit_response=$(add_user_role "$userId" "$caseworker_cmc_judge_id")
                  # Uncomment lines below for batch requiring idam-mfa-disabled role
                  # if [[ $submit_response != *"HTTP-"* ]]; then 
                    # submit_response=$(add_user_role "$userId" "$idam_mfa_disabled")          
                  # fi
                fi
              fi   
            fi
        ...
        if [ $inviteStatus == "SUCCESS" ]; then
          ...
          echo "${total_counter}: ${email}: ${GREEN}${inviteStatus}${NORMAL}"
        else
          # FAIL:
